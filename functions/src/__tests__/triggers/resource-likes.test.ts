import { onResourceLikeCreate, onResourceLikeDelete } from '../../triggers/resource-likes';
...
describe('Resource Likes Triggers', () => {
  ...
  describe('onResourceLikeCreate', () => {
    it('should increment likes counter when resource like is created', async () => {
      const likeData = {
  ...
  describe('onResourceLikeDelete', () => {
    it('should decrement likes counter when resource like is deleted', async () => {
      const likeData = {
        resourceId: 'resource123',
        userId: 'user456',
      };
    ...
    it('should handle errors during decrement', async () => {
      ...
      const snap = mockDocumentSnapshot(likeData, 'test-id', 'resource-likes');
      (incrementCounter as jest.Mock).mockRejectedValueOnce(new Error('Decrement failed'));
